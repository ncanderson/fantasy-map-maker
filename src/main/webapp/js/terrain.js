...
function cityScore(h, cities, params) {
    var score = map(getSlope(h), Math.sqrt);
    for (var i = 0; i < h.length; i++) {
        if (isnearedge(h.mesh, i)) {
            score[i] = -999999;
            continue;
        ...
        for (var j = 0; j < cities.length; j++) {
            score[i] -= params.citySpacing / (distance(h.mesh, cities[j], i) + 1e-9);
        }
    }
    return score;
}
function placeCity(render) {
    ...
    var score = cityScore(render.h, render.cities, render.params);
...
function visualizeSlopes(svg, render) {
    ...
    for (var i = 0; i < h.length; i++) {
        ...
        s /= nbs.length;
        s2 /= nbs.length;
        if (Math.abs(s) < runif(0.1, 0.3)) continue;
...
function generateCoast(params) {
    ...
    var h = add(
            ...
            cone(mesh, runif(-1, -1)),
            mountains(mesh, params.mountainPeaks, params.mountainHeight)
            );
    for (var i = 0; i < 5; i++) {
        h = relax(h);
    }
    ...
    h = setSeaLevel(h, runif(params.waterLevelMin, params.waterLevelMax));
    h = fillSinks(h);
    h = cleanCoast(h, 3);
    return h;
}
...
function doMap(svg, params) {
    ...
    svg.attr('width', params.canvasSize.width);
    var width = params.canvasSize.width;
...
var defaultParams = {
    ...
    /* water level from .1 - 1 (low to high) */
    waterLevelMin: .2,
    waterLevelMax: .3,
    
    /* city spacing - .002 is stock, .0015 is significantly different: lower is closer */
    citySpacing: .002,
    ...
    /*
	 * default npts: 16384, ncities: 15, nterrs: 5,
	 */
    fontsizes: {
        region: 50,
        city: 30,
